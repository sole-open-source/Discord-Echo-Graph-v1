...
# logger = get_logger(module_name="partial_response", DIR="Educhatv2")
...
async def process_single_chunk(llm : BaseChatModel, semaphore : asyncio.Semaphore, prompt : str, data_chunk : MessageChunk) -> ProccesSingleChunk:
    ...
    async with semaphore:
        try:
            ...
            print(f"error procesando los mensjes recuperados del canal con id {data_chunk['channel_id']}: \n {e} \n\n")
            return None



async def generate_partial_responses(semaphore : asyncio.Semaphore, query : str, llm : BaseChatModel, chunks : List[MessageChunk]):
    tasks = [
